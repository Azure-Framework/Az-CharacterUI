...
-- =========================================================
-- fivem-appearance customization convar sync (server-side)
-- =========================================================
...
exports("getActiveCharacter", function(src)
...
local appearanceCache  = {}  -- [did|charid]={appearance,at}
...
-- =========================================================
-- DB driver helpers (oxmysql OR mysql-async)
-- =========================================================
...
local function getDiscordID(src)
  ...
  -- fallback: numeric identifier sometimes present
  for _, id in ipairs(ids) do
...
local function tryResolveHouseDoorCoords_DB_ONLY(houseRow)
  ...
  -- fallback to interior spawn mapping or fallback spawn
...
-- =========================================================
-- Spawns (file + dynamic last location + house)
-- =========================================================
local spawns = {}
...
RegisterNetEvent("spawn_selector:requestSpawns", function(optionalCharId)
  ...
  -- optionally accept char id and set activeCharacters (validated)
...
RegisterNetEvent("spawn_selector:saveSpawns", function(payload)
  ...
  -- filter out last location injections (never saved)
  local filtered = {}
  for i = 1, #list do
    local s = list[i]
    if type(s) == "table" then
...
-- =========================================================
-- Select character (active char + push appearance)
-- =========================================================
...
-- =========================================================
-- Final save on shutdown / resource stop
-- =========================================================
...
AddEventHandler("playerDropped", function(reason)
  ...
  -- best-effort lastpos save on drop
  ...
  -- prune appearance cache for this discord
  if did ~= "" then
    purgeAppearanceCacheForDid(did)
  end
end)
